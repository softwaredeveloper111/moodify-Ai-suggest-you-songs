...
export default function MoodGraph({ mood }) {
  ...
  const safeMood = MOOD_CFG[mood] ? mood : "neutral";
  ...
  useEffect(() => {
    ...
    const entry = { mood: safeMood, time: new Date().toISOString() };
  ...
  const weekData = last7.map((day) => {
    ...
    const counts = entries.reduce(
      (acc, e) => ({ ...acc, [e.mood]: (acc[e.mood] || 0) + 1 }),
      {},
    );
    const dominant =
  ...
  const currentCfg = MOOD_CFG[safeMood] || MOOD_CFG.neutral;
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            />
            <span
              style={{ fontSize: 10, color: currentCfg.color, fontWeight: 600 }}
            >
              LIVE
            </span>
          </div>
        ...
        <div className="mg-tabs">
          <button
            className={`mg-tab ${view === "today" ? "on" : "off"}`}
            onClick={() => setView("today")}
          >
            Today
          </button>
          <button
            className={`mg-tab ${view === "week" ? "on" : "off"}`}
          ...
          (todayEntries.length === 0 ? (
            ...
            <div className="mg-timeline">
              {[...todayEntries].reverse().map((e, i) => {
                const cfg = MOOD_CFG[e.mood] || MOOD_CFG.neutral;
            ...
            {weekData.map((d, i) => {
              ...
              return (
                <div key={d.day} className="mg-daycol">
                  ...
                  <div
                    className="mg-dayname"
                    style={{ color: isToday ? "#22d3ee" : "#334155" }}
                  >
                    {d.day}
                  </div>
