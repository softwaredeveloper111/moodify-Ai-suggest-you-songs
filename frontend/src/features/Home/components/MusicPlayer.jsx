...
export default function MusicPlayer({
  ...
  const safeMood = moodColors[song.mood] ? song.mood : "neutral";
  const colors = moodColors[safeMood] || moodColors.neutral;
  const title = song.title || "Untitled Song";
  const artist =
    typeof song.artist === "string" ? song.artist : "Unknown Artist";
  const artistName = artist.split(",")[0].trim() || "Unknown Artist";
  ...
  const goPrev = () => {
    ...
    setIdx((p) => (p - 1 + songs.length) % songs.length);
    setPlaying(true);
  };
  const goNext = () => {
    setIdx(
      ...
      >
        <div className="mp-glass">
          <div className="mp-cover">
            {!imgErr[song._id] && song.posterUrl ? (
              <img
                key={song._id}
                src={song.posterUrl}
          ...
          <div className="mp-right">
            <div className="mp-toprow">
              <span className="mp-badge">{safeMood}</span>
              <button
                className="mp-like"
                onClick={() =>
                  setLiked((p) => ({ ...p, [song._id]: !p[song._id] }))
                }
              >
            ...
            <div>
              <div className="mp-title" title={title}>
                {title}
              </div>
              <div className="mp-artist">{artistName}</div>
            </div>
            <div
              className="mp-seek"
              ref={seekRef}
            ...
            <div className="mp-controls">
              <button
                className={`mp-ic${shuffle ? " on" : ""}`}
                onClick={() => setShuffle((s) => !s)}
              >
                <Shuffle />
              </button>
              <button className="mp-nav" onClick={goPrev}>
                <Prev />
              </button>
              <button className="mp-playbtn" onClick={togglePlay}>
                {playing ? <Pause /> : <Play />}
              </button>
              <button className="mp-nav" onClick={goNext}>
                <Next />
              </button>
              <button
                className={`mp-ic${repeat !== "none" ? " on" : ""}`}
                onClick={() =>
                  setRepeat((r) =>
            ...
            <div className="mp-vol">
              <button className="mp-volic" onClick={() => setMuted((m) => !m)}>
                <VolIcon muted={muted} v={volume} />
              </button>
              <div className="mp-voltrack" ref={volRef} onClick={changeVol}>
                <div
                  className="mp-volfill"
                  style={{ width: `${muted ? 0 : volume * 100}%` }}
                />
              </div>
              <span className="mp-volnum">
